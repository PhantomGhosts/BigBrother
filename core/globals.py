...
class vars:
    ...
    with open(os.path.join(os.path.split(os.path.abspath(__file__))[0], \
        'vars.yml'), 'r') as yaml_config_file:
...
#         ("language", ("c", "cpp", "asm", "bin", "java", "apk", "vb", "php"))]
    ...
    banner += bold('''      _,,__  ''' + red('''B 9_ "v,''')) + bold(blue('''_Zp*"""""^@u# P''')) + bold(red(''' _m^"^u''')) + bold(''',a*"   j    ''') + '\tBigBrother: %s\n' % bold(green(version))
    banner += bold('''    _F    `''' + red('''4 A_ "*-''')) + bold(blue('''ap"            ^Lj"''')) + bold(red(''' _smu,''')) + bold('''    _* ,   ''') +  '\tCodename: %s\n' % bold(red(codename))
    banner += bold('''    "__,,_ ''' + red('''jL  -- ''')) + bold(blue('''m<                5j!''')) + bold(red(''' ____*''')) + bold('''-*^   &   ''')  + '\tAuthor: %s\n' % bold(author)
